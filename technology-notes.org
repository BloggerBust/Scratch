...
*** KVM
    ...
    - The command to create a kvm on a block device
      #+BEGIN_SRC bash
      # qemu-system-x86_64 -enable-kvm -global ide-drive.physical_block_size=4096 -drive file=tank/vm/prod-web-1,if=ide -machine pc,accel=kvm,iommu=on -cpu host -smp 4
      #+END_SRC
** xfreerdp
